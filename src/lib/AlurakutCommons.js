  ...
  return (
    <AlurakutMenu.Wrapper isMenuOpen={isMenuOpen}>
      <div className="container">
        ...
        <nav style={{ flex: 1 }}>
          {[{ name: 'Home', slug: '/login'}, {name: 'Friends', slug: '/friends'}, {name: 'Comunities', slug: '/comunities'}].map((menuItem) => (
  ...
  return (
    <OrkutNostalgicIconSet.List>
      {[
        ...
        { name: 'Pictures', slug: 'pictures', icon: 'camera' },
        ...
        <li key={`orkut__icon_set__${slug}`}>
          ...
          <span className="OrkutNostalgicIconSet__number" style={{ gridArea: 'number' }}>
            ...
            {props[slug] ? props[slug] : 25}
          </span>
        </li>
      ))}
      {[
        { name: 'Reliable', slug: 'reliable', icon: 'smile' },
        { name: 'Cool', slug: 'cool', icon: 'cool' },
        { name: 'Sexy', slug: 'sexy', icon: 'heart' },
      ].map(({ name, slug, icon }) => {
        const total = props[slug] ? props[slug] : 10;
        return (
          <li key={`orkut__icon_set__${slug}`}>
